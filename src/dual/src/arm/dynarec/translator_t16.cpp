...
ir::Function* TranslatorT16::TransFun(atom::Arena& memory_arena, u32 r15, ir::Mode cpu_mode) {
  const u32 function_entrypoint = r15 - sizeof(u16) * 2u;

  Analyzer analyzer{m_memory};
  std::vector<Analyzer::BlockOfCode> blocks_of_code = analyzer.AnalyzeT16(function_entrypoint);
  ...
  for(const auto& block_of_code : blocks_of_code) {
    ...
    switch(link.type) {
      case Analyzer::Link::Type::Exit: fmt::print("exit"); break;
      case Analyzer::Link::Type::UnconditionalBranch: fmt::print("b 0x{:08X}", link.targets[0]); break;
      case Analyzer::Link::Type::ConditionalBranch: fmt::print("bcc {} 0x{:08X} 0x{:08X}", (int)link.condition, link.targets[0], link.targets[1]); break;
  ...
  for(const auto& block_of_code : blocks_of_code) {
    ...
    for(u32 address = block_of_code.address_lo; address <= block_of_code.address_hi; address += sizeof(u16)) {
      Code code = Translate(address + sizeof(u16) * 2u, cpu_mode,  m_memory.ReadHalf(address, Memory::Bus::Code), emitter);
      if(code != Code::Success) {
        ATOM_PANIC("could not compile, got unsupported instruction");
      }
    }

    const Analyzer::Link& link = block_of_code.link;
    switch(link.type) {
      case Analyzer::Link::Type::Exit: {
        emitter.EXIT();
        break;
      }
      case Analyzer::Link::Type::UnconditionalBranch: {
        emitter.BR(*address_to_basic_block[link.targets[0]]);
        break;
      }
      case Analyzer::Link::Type::ConditionalBranch: {
